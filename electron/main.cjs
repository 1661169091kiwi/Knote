...
const isProbe = !!(process.env.KNOTE_SMOKE || process.env.KNOTE_SHOT)
...
if (!gotLock) {
  ...
  ipcMain.handle('knote:fs-list', (_e, { dir }) => {
    if (!insideRoot(dir)) throw new Error('outside workspace')
  ...
  ipcMain.handle('knote:fs-exists', (_e, { path: p }) => insideRoot(p) && fs.existsSync(p))
        ...
        setTimeout(async () => {
          try {
            ...
            if (opened) {
              ...
              console.log('KNOTE_SMOKE_SAVE:' + JSON.stringify({ liveSaved: onDisk.includes('IPC写盘验证') }))
            }
