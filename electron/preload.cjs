...
contextBridge.exposeInMainWorld('knoteDesktop', {
  ...
  // hardware-acceleration kill switch (crash triage; relaunch to apply)
  hwAccelGet: () => ipcRenderer.invoke('knote:hw-accel-get'),
  hwAccelSet: (disabled) => ipcRenderer.invoke('knote:hw-accel-set', { disabled: !!disabled }),
  relaunchApp: () => ipcRenderer.invoke('knote:app-relaunch'),
  onPdfEnvProgress: (cb) => {
