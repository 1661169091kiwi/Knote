...
test('structurally expensive documents mount one bounded TipTap chunk', () => {
  ...
  assert.match(app, /data-testid="large-document-split-preview"/)
  assert.match(app, /data-testid="large-document-chunk-preview"/)
  assert.match(app, /Ctrl \+ \/[\s\S]*Toggle single \/ split view/)
  assert.match(app, /e\.code === 'Slash'[\s\S]*setViewMode\(viewMode\.value === 'single' \? 'split' : 'single'\)/)
