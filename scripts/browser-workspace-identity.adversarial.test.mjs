...
test('App resolves pathless file identity without using the display filename as workspace identity', () => {
  ...
  assert.ok(open.indexOf('withAsyncKeyLock(mutationKey') < open.indexOf('file = await handle.getFile()'))
  assert.ok(open.indexOf('file = await handle.getFile()') < open.indexOf('installOpenedMarkdown'))
