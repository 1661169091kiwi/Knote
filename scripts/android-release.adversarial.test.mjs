...
test('release workflow validates, tests, verifies, and atomically publishes', () => {
  ...
  const linuxIndex = workflow.indexOf('  linux:')
  const publishIndex = workflow.indexOf('  publish:')
  assert.ok(validateIndex >= 0 && validateIndex < androidIndex)
  assert.ok(androidIndex < windowsIndex && windowsIndex < linuxIndex && linuxIndex < publishIndex)

  const validateJob = workflow.slice(validateIndex, androidIndex)
  const androidJob = workflow.slice(androidIndex, windowsIndex)
  const windowsJob = workflow.slice(windowsIndex, linuxIndex)
  const linuxJob = workflow.slice(linuxIndex, publishIndex)
  ...
  assert.match(linuxJob, /needs:\s*validate/)
  assert.match(linuxJob, /name:\s*knote-linux-release/)
  ...
  assert.match(publishJob, /needs:\s*\[android, windows, linux\]/)
  assert.match(publishJob, /actions\/download-artifact@v4/)
  assert.match(publishJob, /Expected exactly one verified Android APK, one Windows installer, and one of each Linux package \(AppImage\/rpm\/deb\)/)
  ...
  assert.match(publishJob, /Knote-\$\{expected_version\}\.AppImage/)
  assert.match(publishJob, /Knote-\$\{expected_version\}\.rpm/)
  assert.match(publishJob, /Knote-\$\{expected_version\}\.deb/)
  ...
  assert.match(publishJob, /length == 5/)
...
test('release documentation distinguishes local builds and Android native-first transport', () => {
  ...
  assert.match(handoff, /应用版本 `1\.1\.52`/)
