        ...
        <section :key="step" class="onboarding-scene" :class="`is-step-${step}`">
          ...
          <div v-if="step === 0" class="onboarding-product-model" aria-hidden="true">
            ...
            <div class="model-keyboard" aria-label="Keyboard typing animation">
              ...
              <div class="keyboard-row"><i>Z</i><i>X</i><i>C</i><i>V</i><i>B</i><i>N</i><i>M</i><i class="keyboard-space key-press-4"></i></div>
            </div>
...
.model-keyboard{position:absolute;left:24px;right:104px;bottom:-2px;z-index:5;height:102px;box-sizing:border-box;padding:10px 12px;border:1px solid rgba(35,42,33,.11);border-radius:22px;background:rgba(250,252,248,.91);box-shadow:0 22px 45px rgba(49,62,39,.15);backdrop-filter:blur(18px);transform-origin:50% 100%;animation:obKeyboardSequence 3.12s .45s cubic-bezier(.2,.8,.2,1) both}.keyboard-row{height:25px;display:flex;justify-content:center;gap:5px;margin:0 0 4px}.keyboard-row i{display:grid;place-items:center;min-width:24px;height:23px;padding:0 3px;border:1px solid rgba(43,51,40,.1);border-radius:7px;background:rgba(255,255,255,.86);box-shadow:0 2px 0 rgba(54,65,49,.12);color:#788074;font:600 7px/1 ui-monospace,SFMono-Regular,Consolas,monospace;font-style:normal}.keyboard-row .keyboard-space{min-width:90px}.keyboard-row .key-press-1{animation:obKeyPress .22s 1.05s ease}.keyboard-row .key-press-2{animation:obKeyPress .22s 1.38s ease}.keyboard-row .key-press-3{animation:obKeyPress .22s 1.72s ease}.keyboard-row .key-press-4{animation:obKeyPress .22s 2.08s ease}
.model-agent-launcher{position:absolute;right:22px;bottom:12px;z-index:6;width:76px;height:76px;display:grid;place-items:center;border:1px solid rgba(39,46,36,.08);border-radius:25px;background:rgba(255,255,255,.84);box-shadow:0 16px 38px rgba(54,68,42,.15);opacity:0;animation:obLauncherIn .46s 2.86s cubic-bezier(.2,.8,.2,1) both}.model-agent-launcher:after{content:"";position:absolute;inset:7px;border:1px solid rgba(132,204,22,.48);border-radius:19px;opacity:0;animation:obClickRing .5s 3.82s ease-out forwards;pointer-events:none}
