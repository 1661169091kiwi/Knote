...
const TOOLS = [
  ...
  {
    name: 'insert_image',
    description: '把一张图片插入到文档中第 after_line 行之后（0 = 文档开头）。image_id 可以是：用户发送的图片附件、render_pdf_page 页面截图、pdf_crop_region 裁剪图，或 pdf_prepare 提取的元素（el-…）。这是把图片放进文档的【唯一】方式——绝不要在正文里手写 knote-img/att-/el- 引用或留 ![描述] 空占位符。改动暂存为"待审核改动"，用户接受后才生效。',
    parameters: {
      type: 'object',
      properties: {
...
- 图片进入文档的唯一方式是 insert_image 工具。写正文时【不要】留 ![描述] 空占位符、【不要】手写任何 knote-img/att-/el- 引用——先写完该段文字（记住行号），随后立即用 insert_image 在该行号后插入对应元素。
...
const placeholderNote = (n) => (n ? `⚠ 检测到 ${n} 个没有链接的图片占位符（![描述] 形式）——它们不会显示为图片。请对每个占位符位置调用 insert_image 插入真实图片（图/表先用 pdf_prepare 提取拿到 element_id）。` : '')
