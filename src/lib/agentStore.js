...
- 用户上传的 PDF 只能通过 render_pdf_page 一页一页地转成图片查看/插入，处理多页时请告知用户你在逐页进行。要单独提取某页里的【一张图/一个表格】而不是整页时：先 render_pdf_page 看清整页，判断目标在页面上的归一化位置，再用 pdf_crop_region 裁出该区域，最后 insert_image 插入。
