...
- 文档里形如 knote-img:xxx 的图片引用是应用内部的图片指针，保留原样，不要改动。也【绝不要自己编写】这种引用或把 image_id（att-xxx）当作图片地址写进文档——手写的引用没有图片数据，会显示为裂图。把图片放进文档的唯一有效方式是调用 insert_image 工具。
- 数学公式用 $...$ / $$...$$，代码块用围栏语法，与文档现有风格保持一致。
- 读 PDF 按【成本从低到高】选工具：① read_pdf_text 提取文本层（首选，一次≤20页，token 极省）——课件/论文等数字 PDF 都有文本层；② pdf_layout 版面分析（若可用）——本地识别整页内容+图/表位置，适合要图表或文本层为空的页；③ render_pdf_page 渲染成图（一次≤6页）——只在前两者不够时用（扫描件、需要看排版）。处理长 PDF 必须【边读边写】：每读完一批页面，立即把该批的成果写入文档或回复，再读下一批——不要只说"继续看下一批"却不产出任何内容，也不要等全部读完才动笔。
- 要单独提取某页里的【一张图/一个表格】而不是整页时：优先用 pdf_layout 拿到各元素的精确 bbox（若该工具可用），再用 pdf_crop_region 按 bbox 裁出、insert_image 插入；若 pdf_layout 不可用，则先 render_pdf_page 看清整页、自己判断目标的归一化位置，再 pdf_crop_region 裁出。
...
const adoptModelImageRefs = (text) => {
  let out = String(text ?? '')
  // bare `](att-…)` image src → `](knote-img:att-…)` so display pipelines see it
  out = out.replace(/\]\(\s*(att-[\w-]+)\s*\)/g, '](knote-img:$1)')
  const re = /knote-img:(att-[\w-]+)/g
  let m
  while ((m = re.exec(out))) {
    const att = attachmentPool[m[1]]
    if (att && att.kind === 'image' && att.dataUrl && agentBridge.registerImage) {
      agentBridge.registerImage(m[1], att.dataUrl)
    }
  }
  return out
}
...
const summarizeArgs = (call) => {
  try {
    ...
    if (call.name === 'render_pdf_page' || call.name === 'read_pdf_text') return `第 ${Array.isArray(i.pages) && i.pages.length ? i.pages.join('、') : i.page} 页`
