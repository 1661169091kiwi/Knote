...
- 用户上传的 PDF 只能通过 render_pdf_page 一页一页地转成图片查看/插入，处理多页时请告知用户你在逐页进行。要单独提取某页里的【一张图/一个表格】而不是整页时：优先用 pdf_layout 做版面分析拿到各元素的精确 bbox（若该工具可用），再用 pdf_crop_region 按 bbox 裁出、insert_image 插入；若 pdf_layout 不可用或未安装，则先 render_pdf_page 看清整页、自己判断目标的归一化位置，再 pdf_crop_region 裁出。
...
const FOLDER_TOOLS = new Set(['list_files', 'read_file', 'batch_process'])
        ...
        msgs.push({
          ...
          content: results.map(({ call, result }) => ({
            ...
            content: [
              ...
              // makes Anthropic 400. Derive it from the data URL.
              ...((() => {
                const pp = result.imageDataUrl ? dataUrlParts(result.imageDataUrl) : null
                return pp ? [{ type: 'image', source: { type: 'base64', media_type: pp.mediaType, data: pp.base64 } }] : []
              })())
            ]
          }))
        })
