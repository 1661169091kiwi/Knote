...
const TOOLS = [
  ...
  },
  {
    ...
    description: '把用户上传的 PDF 附件渲染成图片。一次最多 6 页（用 pages 数组传多个页码）；更长的 PDF 分多次调用。渲染出的每页图片各获得一个 image_id，可用 insert_image 插入文档；如果当前模型支持视觉，图片也会展示给你。处理长 PDF 请边读边写：每读完一批立即产出该批的内容，再读下一批。',
    parameters: {
      type: 'object',
      properties: {
...
- 用户上传的 PDF 通过 render_pdf_page 转成图片查看/插入，一次最多 6 页（用 pages 数组）。处理长 PDF 必须【边读边写】：每读完一批页面，立即把该批的成果写入文档或回复，然后再渲染下一批——不要只说"继续看下一批"却不产出任何内容，也不要等全部读完才动笔。要单独提取某页里的【一张图/一个表格】而不是整页时：优先用 pdf_layout 做版面分析拿到各元素的精确 bbox（若该工具可用），再用 pdf_crop_region 按 bbox 裁出、insert_image 插入；若 pdf_layout 不可用或未安装，则先 render_pdf_page 看清整页、自己判断目标的归一化位置，再 pdf_crop_region 裁出。
...
const activeTools = () => TOOLS.filter((t) => {
  ...
  if (t.name === 'pdf_layout') return !!(typeof window !== 'undefined' && window.knoteDesktop && window.knoteDesktop.pdfAnalyze)
...
const execReplaceLines = (input) => {
  ...
  return `已暂存改动（${hunkTitle(h)}，hunk_id=${h.id}），${STAGED_NOTE}如内容未输完，可用 continue_hunk 继续追加。`
}
...
const execContinueHunk = (input) => {
  ...
  const more = String(input.content ?? '').replace(/\r\n?/g, '\n').split('\n')
  ...
  syncPreview(h.id)
  return `已追加 ${more.length} 行到待审核改动（${hunkTitle(h)}，hunk_id=${id}）。还有剩余内容就继续调用 continue_hunk，全部写完后再总结。`
}
...
const execRenderPdfPage = async (input) => {
  ...
  try {
    ...
    lines.push('可用 insert_image 插入文档。请先把这批页面的内容处理/写入完，再渲染下一批。')
...
const execPdfCropRegion = async (input) => {
  ...
  try {
    ...
    return {
      text: `已从《${att.name}》第 ${page} 页裁剪出所选区域（image_id=${img.id}）。可用 insert_image 把它插入文档。`,
...
const execPdfLayout = async (input) => {
  ...
  try {
    ...
    const lines = els.map((e) => `- [${e.id}] ${e.type}  bbox=[${e.bbox.join(', ')}]${e.text ? '  “' + String(e.text).slice(0, 36) + '”' : ''}`)
    return `《${att.name}》第 ${page} 页版面分析：检测到 ${els.length} 个元素（bbox 为归一化 [x0,y0,x1,y1]，左上原点）：\n${lines.join('\n')}\n\n要把其中的某张图/表插入文档：用 pdf_crop_region(page=${page}, bbox=该元素的 bbox) 裁出，再 insert_image 插入。`
  } finally {
    if (task) await task.destroy()
    pdfProcessing.value = null
  }
...
const executeTool = async (name, input, signal) => {
  switch (name) {
    ...
    case 'create_file': {
      ...
      const out = await agentBridge.writeFile(p, String(input.content ?? ''))
      return { text: out ? `已创建文件「${out}」（未覆盖任何已有文件）。用户可在文件树中打开它。` : '错误：创建文件失败（路径可能无效）。' }
    }
    case 'create_folder': {
...
2) 该调用的工具是否调用了——例如要求"总结/修改文档"却没有调用 read_document 读取原文、要求处理 PDF 却没有 render_pdf_page、要求跨文件却没有 read_file/list_files，都算缺失；
...
export const sendToAgent = async (text, atts, extra) => {
  ...
  try {
    ...
    // judge only the current pass's answer (the bubble is reset between passes,
    // so liveMsg().text is exactly this pass's output, never an accumulation)
    ...
    trace.push({ name: '__verify', label: '自查：需补做' + ((verdict.missing_actions && verdict.missing_actions.length) ? ' ' + verdict.missing_actions.join('、') : ''), done: true })
    pushAssistant()
    // reset the visible bubble so the corrected pass REPLACES the rejected
    // answer (rather than gluing them together) and isn't re-judged/duplicated
    liveMsg().text = ''
    }
...
const summarizeArgs = (call) => {
  try {
    ...
    if (call.name === 'render_pdf_page') return `第 ${Array.isArray(i.pages) && i.pages.length ? i.pages.join('、') : i.page} 页`
